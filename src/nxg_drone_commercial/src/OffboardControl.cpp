...
void OffboardControl::setVehicleLocalPosition(px4_msgs::msg::VehicleLocalPosition msg) {
    ...
    //printf("Vehicle Local Position (%.4f,%.4f,%.4f)\n", this->vehicle_position.x_m, this->vehicle_position.y_m, this->vehicle_position.z_m);
}
...
void OffboardControl::publishVIO(nav_msgs::msg::Odometry msg) {
    ...
    vio_msg.quality = 0;
    vio_msg.timestamp = static_cast<uint64_t>(this->get_clock()->now().nanoseconds()) / static_cast<uint64_t>(1000);
